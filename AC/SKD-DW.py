...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_star_inner, u_star_inner, X_f, layers, lb, ub):
        ...
        self.A_T = 0.65
        self.temp_1 = np.ones(len(X_f[:, 0:1]))
        ...
        self.u_nl_pred, _ = self.net_u(self.x_f_tf, self.t_f_tf)
        self.f_pred = self.net_f(self.x_f_tf, self.t_f_tf)  # f预测和u预测的输入都是全部的x和t
        ...
        self.e = tf.abs(self.f_pred)
        # self.reg_loss = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)

        self.loss = (1 - self.alpha_tf) * tf.reduce_mean(tf.square(self.e)) + \
                    self.alpha_tf * tf.reduce_mean(
            tf.multiply(tf.convert_to_tensor(np.array(self.temp_1, dtype='float32')) - tanh_test(self.r_t_1_tf),
                        tf.square(self.u_nl_pred - self.u_t_1_tf))) + tf.reduce_mean(
    ...
    def train(self, nIter):
        u_t_1 = np.zeros(len(self.x_f))
        u_t_1 = np.reshape(u_t_1, (len(self.x_f), 1))
        r_t_1 = np.zeros(len(self.x_f))
        r_t_1 = np.reshape(r_t_1, (len(self.x_f), 1))
        alpha = 0.0
        ...
        for it in range(nIter):
            ...
            p_c, r = self.sess.run([self.u_nl_pred, self.e], tf_dict)
...
if __name__ == "__main__":
    ...
    x0 = x[idx_x, :]
    u0 = Exact[idx_x, 0:1]
    idx_t = np.random.choice(t.shape[0], 100, replace=False)  # idx_t边界条件对应的随机选择的t的50个值 后面是对应的解
    tb = t[idx_t, :]
    ...
    np.savetxt("skd_DW.csv", u_predict, delimiter=",")